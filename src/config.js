...
const CONFIG = {
  ...
  // RG9:装备词条重做——每件装备是独立"掉落实例"(不再是"槽位+品阶"就能唯一确定的固定值):
  //   主属性:类型固定为这件装备槽位本身对应的属性,数值降到 gearValues 原表的 mainMult,再在 [mainMin,mainMax] 区间内浮动;
  //   副属性:3条,每条独立随机抽一个槽位类型(8选1,允许和主属性/其它副属性重复,"完全任意"),数值是"那个类型在这件
