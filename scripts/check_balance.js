...
assert.strictEqual(CONFIG.endless.enemyHpBaseMult, 1.25, "endless enemy base HP should leave room for player growth");
assert.strictEqual(CONFIG.endless.enemyHpBoostTime, 60, "endless enemies should jump to the target boost at 60s");
assert.strictEqual(CONFIG.endless.enemyHpBoostMult, 2.2, "endless enemies should use a lower 60s HP boost");
assert.strictEqual(CONFIG.endless.enemyHpDoubleInterval, 180, "endless enemy HP multiplier should grow without creating a wall");
...
assert.strictEqual(CONFIG.endless.enemyHpFloor, 220, "opening enemy HP floor should avoid outpacing player growth");
...
assert.strictEqual(CONFIG.endless.enemyHpFloorTarget, 8000, "200s enemy HP floor should match strong-route TTK target");
assert.strictEqual(CONFIG.endless.enemyHpFloorMax, 16000, "late enemy HP floor should cap before becoming impossible");
...
between(hellFloorAt40, 4, 15, "40s hell enemy floor should avoid a sudden HP wall");
...
between(game.endlessEnemyHpFloor() / focusedDps.fourPick, 0.8, 2, "60s enemy floor TTK");
...
assert(Math.abs(game.endlessEnemyHpMult() - CONFIG.endlessDifficulties.hell.enemyHpBoostMult * 4) < 1e-9, "endless enemies should keep doubling on the slower curve");
...
between(game.endlessEnemyHpFloor() / focusedDps.strongRoute, 5, 7, "200s enemy floor TTK for strong focused fire");
game._endlessT = 420;
between(game.endlessEnemyHpFloor(), 11000, CONFIG.endless.enemyHpFloorMax, "420s enemy floor");
between(game.endlessEnemyHpFloor() / focusedDps.nearCap, 4, 6, "420s enemy floor TTK for near-cap focused fire");
